...
"[project]/frontend/node_modules/.pnpm/@meshsdk+common@1.9.0-beta.87/node_modules/@meshsdk/common/dist/index.js [app-client] (ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
var mAssetClass = (currencySymbolHex, tokenNameHex)=>{
    if (currencySymbolHex.length !== 56 && currencySymbolHex !== "") {
        throw new Error(`Invalid policy id for [${currencySymbolHex}] - should be ${POLICY_ID_LENGTH} bytes long or empty string for lovelace`);
    }
    if (tokenNameHex.length > 64) {
        throw new Error(`Invalid asset name for [${tokenNameHex}] - should be less than 32 bytes (64 hex length) long`);
    }
    return mConStr0([
        currencySymbolHex,
        tokenNameHex
...
var mSome = (value2)=>mConStr0([
        value2
    ]);
var mNone = ()=>mConStr1([]);
// src/data/mesh/credentials.ts
var mVerificationKey = (bytes)=>mConStr0([
        bytes
    ]);
var mScript = (bytes)=>mConStr1([
        bytes
    ]);
var mMaybeStakingHash = (stakeCredential, isStakeScriptCredential = false)=>{
    ...
    if (isStakeScriptCredential) {
        return mConStr0([
            mConStr0([
                mScript(stakeCredential)
            ])
        ]);
    }
    return mConStr0([
        mConStr0([
            mVerificationKey(stakeCredential)
        ])
    ]);
};
    ...
    if (validation) {
        pList.forEach((item)=>{
            if (typeof item !== "object") {
                throw new Error("List item of JSON Cardano data type must be an object - " + item);
            }
        });
    }
    return {
...
var stringToBSArray = (hexString)=>{
    ...
    return list(processRawStringIntoPlutusByteArray(hexString).map(builtinByteString));
};
        ...
        map: mapItems.map(([k, v])=>{
            if (validation) {
                if (typeof k !== "object" || typeof v !== "object") {
                    throw new Error(`Map item of JSON Cardano data type must be an object - ${k}, ${v}`);
                }
            }
            return {
                k,
...
var hashByteString = (bytes)=>{
    if (bytes.length !== 56) {
        throw new Error(`Invalid hash for [${bytes}] - should be 28 bytes (56 hex length) long`);
    }
    return byteString(bytes);
};
...
var policyId = (bytes)=>{
    if (bytes.length !== POLICY_ID_LENGTH && bytes !== "") {
        throw new Error(`Invalid policy id for [${bytes}] - should be ${POLICY_ID_LENGTH / 2} bytes (${POLICY_ID_LENGTH} hex length) long or empty string for lovelace`);
    }
    return byteString(bytes);
};
...
var assetName = (bytes)=>{
    if (bytes.length > 64) {
        throw new Error(`Invalid asset name for [${bytes}] - should be less than 32 bytes (64 hex length) long`);
    }
    return byteString(bytes);
};
var tokenName = (bytes)=>assetName(bytes);
var assetClass = (currencySymbolHex, tokenNameHex)=>conStr0([
        currencySymbol(currencySymbolHex),
...
var some = (value2)=>conStr0([
        value2
    ]);
var none = ()=>conStr1([]);
// src/data/json/credentials.ts
var verificationKey = (bytes)=>conStr0([
        pubKeyHash(bytes)
    ]);
var script = (bytes)=>conStr1([
        scriptHash(bytes)
    ]);
var maybeStakingHash = (stakeCredential, isStakeScriptCredential = false)=>{
    ...
    if (isStakeScriptCredential) {
        return conStr0([
            conStr0([
                script(stakeCredential)
            ])
        ]);
    }
    return conStr0([
        conStr0([
            verificationKey(stakeCredential)
        ])
    ]);
};
...
var jsonProofToPlutusData = (proof)=>{
    ...
    proofJson.forEach((proof2)=>{
        ...
        switch(proof2.type){
            case "branch":
                proofSteps.push(conStr0([
                    skip,
                    byteString(proof2.neighbors.toString("hex"))
                ]));
                break;
            case "fork":
...
var stringToHex = (str)=>__TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$3_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$buffer$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Buffer"].from(str, "utf8").toString("hex");
var isHexString = (hex)=>/^[0-9A-F]*$/i.test(hex);
var hexToString = (hex)=>__TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$3_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$buffer$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Buffer"].from(hex, "hex").toString("utf8");
var toBytes = (hex)=>{
    if (hex.length % 2 === 0 && /^[0-9A-F]*$/i.test(hex)) return __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$3_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$buffer$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Buffer"].from(hex, "hex");
    return __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$3_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$buffer$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Buffer"].from(hex, "utf-8");
};
var fromUTF8 = (utf8)=>{
    if (utf8.length % 2 === 0 && /^[0-9A-F]*$/i.test(utf8)) return utf8;
    return bytesToHex(__TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$3_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$buffer$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Buffer"].from(utf8, "utf-8"));
};
var toUTF8 = (hex)=>__TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$3_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$buffer$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Buffer"].from(hex, "hex").toString("utf-8");
...
var SLOT_CONFIG_NETWORK = {
    ...
    // Starting at Shelley era
    /** Customizable slot config (Initialized with 0 values). */ testnet: {
        zeroTime: 0,
        zeroSlot: 0,
        slotLength: 0,
        startEpoch: 0,
...
var resolveFingerprint = (policyId2, assetName2)=>{
    return AssetFingerprint.fromParts(toBytes(policyId2), toBytes(assetName2)).fingerprint();
};
var DATA = "asset";
var AssetFingerprint = class _AssetFingerprint {
    ...
    static fromParts(policyId2, assetName2) {
        const hashBuf = (0, __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$blake2b$40$2$2e$1$2e$4$2f$node_modules$2f$blake2b$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"])(20).update(new Uint8Array([
            ...policyId2,
            ...assetName2
        ])).digest("binary");
    ...
    static fromBech32(fingerprint) {
        const { prefix, words } = __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$bech32$40$2$2e$0$2e$0$2f$node_modules$2f$bech32$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["bech32"].decode(fingerprint);
        if (prefix !== DATA) {
            throw new Error("Invalid asset fingerprint");
        }
        const hashBuf = __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$3_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$buffer$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Buffer"].from(__TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$bech32$40$2$2e$0$2e$0$2f$node_modules$2f$bech32$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["bech32"].fromWords(words));
        return _AssetFingerprint.fromHash(hashBuf);
    }
    fingerprint() {
        const words = __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$bech32$40$2$2e$0$2e$0$2f$node_modules$2f$bech32$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["bech32"].toWords(this.hashBuf);
        return __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$bech32$40$2$2e$0$2e$0$2f$node_modules$2f$bech32$2f$dist$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["bech32"].encode(DATA, words);
    }
    hash() {
        return __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$3_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$buffer$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Buffer"].from(this.hashBuf).toString("hex");
    }
    prefix() {
        return DATA;
    }
...
var hashDrepAnchor = (jsonLD)=>{
    const jsonHash = (0, __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$blakejs$40$1$2e$2$2e$1$2f$node_modules$2f$blakejs$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["blake2b"])(JSON.stringify(jsonLD, null, 2), void 0, 32);
    return __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$3_$40$babel$2b$core$40$7$2e$28$2e$5_react$2d$dom$40$19$2e$2$2e$0_react$40$19$2e$2$2e$0_$5f$react$40$19$2e$2$2e$0$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$buffer$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Buffer"].from(jsonHash).toString("hex");
};
// src/utils/file.ts
...
var experimentalSelectUtxos = (requiredAssets, inputs, threshold)=>{
    ...
    totalRequiredAssets.set("lovelace", String(Number(totalRequiredAssets.get("lovelace") || 0) + Number(threshold)));
    ...
    for (const assetUnit of totalRequiredAssets.keys()){
        if (Number(totalRequiredAssets.get(assetUnit)) > 0) {
            ...
            console.warn("Remaining quantity", Number(totalRequiredAssets.get(assetUnit)));
            return [];
        }
    }
    const selectedUtxos = [];
...
var enoughValueHasBeenSelected = (selection, assets)=>{
    return Array.from(assets, (asset)=>({
        ...
        return selection.filter((utxo)=>{
            ...
            const utxoQuantity = utxo.output.amount.reduce((quantity, a)=>quantity.checkedAdd(BigNum.new(asset.unit === a.unit ? a.quantity : "0")), BigNum.new("0"));
...
var largestLovelaceQuantity = (utxoA, utxoB)=>{
    const aLovelaceQuantity = BigNum.new(utxoA.output.amount.find((asset)=>asset.unit === "lovelace")?.quantity ?? "0");
    const bLovelaceQuantity = BigNum.new(utxoB.output.amount.find((asset)=>asset.unit === "lovelace")?.quantity ?? "0");
...
var remainingLovelace = (quantity, initialUTxOSet)=>{
    ...
    const requestedOutputSet = /* @__PURE__ */ new Map([
        [
            "lovelace",
            quantity
        ]
...
var keepRelevant = (requiredAssets, inputs, threshold = "5000000")=>{
    const requestedLovelace = BigNum.new(requiredAssets.get("lovelace") ?? "0").checkedAdd(BigNum.new(threshold));
    const multiAsset = inputs.filter((utxo)=>utxo.output.amount.filter((asset)=>asset.unit !== "lovelace").some((asset)=>requiredAssets.has(asset.unit)));
    const selectedLovelace = selectedLovelaceQuantity(multiAsset);
    const lovelace = selectedLovelace.lessThan(requestedLovelace) ? remainingLovelace(requestedLovelace.clampedSub(selectedLovelace).toString(), inputs.filter((input)=>{
        return !multiAsset.some((selectedUtxo)=>selectedUtxo.input.txHash === input.input.txHash && selectedUtxo.input.outputIndex === input.input.outputIndex);
    })) : [];
    return [
        ...lovelace,
        ...multiAsset
...
var UtxoSelection = class {
    ...
    largestFirst(requiredAssets, inputs) {
        ...
        return largestFirst(requiredAssetWithThreshold.toString(), inputs, this.includeTxFees);
    }
    ...
    largestFirstMultiAsset(requiredAssets, inputs) {
        const lovelaceAmount = requiredAssets.get("lovelace") ?? "0";
        requiredAssets.set("lovelace", (BigInt(lovelaceAmount) + BigInt(this.threshold)).toString());
...
var TxTester = class {
   ...
   */ validAfter = (requiredTimestamp)=>{
        ...
        if (!isValidAfter) {
            this.addTrace("validAfter", `tx invalid before ${invalidBefore}, with requiredTimestamp ${requiredTimestamp}`);
        }
        return this;
    };
    /**
   ...
   */ validBefore = (requiredTimestamp)=>{
        ...
        if (!isValidBefore) {
            this.addTrace("validBefore", `tx invalid after ${invalidHereafter}, with requiredTimestamp ${requiredTimestamp}`);
        }
        return this;
    };
   ...
   */ oneOfKeysSigned = (keyHashes)=>{
        const isOneOfKeysSigned = keyHashes.some((keyHash)=>keySignedLogic(this.txBody.requiredSignatures, keyHash));
        if (!isOneOfKeysSigned) {
            this.addTrace("oneOfKeysSigned", `tx does not have any of the keys signed: ${keyHashes.join(", ")}`);
        }
        return this;
    };
    /**
   ...
   */ allKeysSigned = (keyHashes)=>{
        ...
        if (!isAllKeysSigned) {
            this.addTrace("allKeysSigned", `tx does not have all keys signed: ${missingKeys.join(", ")}`);
        }
        return this;
    };
    /**
   ...
   */ tokenMinted = (policyId2, assetName2, quantity)=>{
        const isTokenMinted = tokenMintedLogic(this.txBody.mints, policyId2, assetName2, quantity);
        if (!isTokenMinted) {
            this.addTrace("tokenMinted", `Token with policy_id: ${policyId2}, asset_name: ${assetName2}, quantity: ${quantity} not found in mints.`);
        }
        return this;
    };
    /**
   ...
   */ onlyTokenMinted = (policyId2, assetName2, quantity)=>{
        ...
        if (!isTokenMinted) {
            this.addTrace("onlyTokenMinted", `Token with policy_id: ${policyId2}, asset_name: ${assetName2}, quantity: ${quantity} not found in mints`);
        }
        if (!isOnlyOneMint) {
            this.addTrace("onlyTokenMinted", `Expected only one mint, but found ${this.txBody.mints?.length || 0} mints.`);
        }
        return this;
    };
    /**
   ...
   */ policyOnlyMintedToken = (policyId2, assetName2, quantity)=>{
        ...
        if (!isOnlyOneMint) {
            this.addTrace("policyOnlyMintedToken", `Expected only one mint for policy_id: ${policyId2}, but found ${filteredMints.length} mints.`);
        }
        if (!isTokenMinted) {
            this.addTrace("policyOnlyMintedToken", `Token with policy_id: ${policyId2}, asset_name: ${assetName2}, quantity: ${quantity} not found in mints.`);
        }
        return this;
    };
    /**
   ...
   * @returns The TxTester instance for chaining
   */ inputsAt = (address)=>{
        this.inputsEvaluating = this.txBody.inputs?.filter((input)=>txInToUtxo(input.txIn).output.address === address) || [];
        return this;
    };
    /**
   ...
   */ inputsWith = (unit)=>{
        this.inputsEvaluating = this.txBody.inputs?.filter((input)=>{
            const inputValue = MeshValue.fromAssets(txInToUtxo(input.txIn).output.amount);
   ...
   */ inputsValue = (expectedValue)=>{
        ...
        if (!isValueCorrect) {
            this.addTrace("inputsValue", `inputs ${JSON.stringify(this.inputsEvaluating)} have value ${JSON.stringify(value2)}, expect ${JSON.stringify(expectedValue)}`);
        }
        return this;
    };
    // /**
   ...
   */ outputsValue = (expectedValue)=>{
        ...
        const isValueCorrect = value2.eq(expectedValue);
        if (!isValueCorrect) {
            this.addTrace("outputsValue", `tx outputs ${JSON.stringify(this.outputsEvaluating)} have value ${JSON.stringify(value2)}, expected ${JSON.stringify(expectedValue)}`);
        }
        return this;
    };
    /**
   ...
   */ outputsInlineDatumExist = (datumCbor)=>{
        ...
        if (outputsWithInlineDatum.length === 0) {
            this.addTrace("outputs_inline_datum_exist", `No outputs with inline datum matching: ${datumCbor}`);
        }
        return this;
    };
};
...
function tokenMintedLogic(mints, policyId2, assetName2, quantity) {
    return mints?.some((token)=>{
        return token.policyId === policyId2 && token.mintValue.findIndex((m)=>m.assetName === assetName2 && BigInt(m.amount) === BigInt(quantity)) >= 0;
    }) || false;
}
;
;
}),
"[project]/frontend/node_modules/.pnpm/@meshsdk+common@1.9.0-beta.68/node_modules/@meshsdk/common/dist/index.js [app-client] (ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
//# sourceMappingURL=9e883__pnpm_a4e2bfbc._.js.map